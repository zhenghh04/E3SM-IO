...
/*----< add_decomp() >-------------------------------------------------------*/
static int add_decomp (e3sm_io_driver *driver, int ncid, const char *infname, int label) {
	char *buf, name[128], *map;
	std::string str;
	FILE *fd;
	...
	err = driver->put_att (ncid, varid[0], "description", NC_CHAR, strlen (str.c_str ()),
						   str.c_str ());
	ERR
	...
	if (raw_decom) {
		...
		err = driver->put_att (ncid, varid[3], "description", NC_CHAR, strlen (str.c_str ()),
							   str.c_str ());
		ERR
	}
	...
	err = driver->put_att (ncid, varid[1], "description", NC_CHAR, strlen (str.c_str ()),
						   str.c_str ());
	ERR
	...
	err = driver->put_att (ncid, varid[2], "description", NC_CHAR, strlen (str.c_str ()),
						   str.c_str ());
	ERR
	...
	if (raw_decom) {
		...
		err = driver->put_att (ncid, varid[4], "description", NC_CHAR, strlen (str.c_str ()),
							   str.c_str ());
		ERR
	}
	...
	{
		int dims_C_int = (int)(*dims_C);
		err			   = driver->put_att (ncid, NC_GLOBAL, name, NC_INT, ndims, &dims_C_int);
		ERR
	}
	free (dims_C);
	...
	start = 0;
	count = nprocs;
	err	  = driver->put_vara (ncid, varid[0], MPI_INT, &start, &count, nreqs, coll);
	ERR
	...
	if (raw_decom) {
		start = 0;
		count = nprocs;
		err	  = driver->put_vara (ncid, varid[3], MPI_INT, &start, &count, raw_nreqs, coll);
		ERR
	}
...
int main (int argc, char **argv) {
	...
	MPI_Info info;
	e3sm_io_driver *driver = NULL;	// HDF5 driver
	e3sm_io_config dummy;			// Dummy config for driver init
	...
	while ((i = getopt (argc, argv, "hvro:l:i:")) != EOF) switch (i) {
			case 'v':
				verbose = 1;
				break;
			case 'o':
	...
	dummy.api = hdf5;
	dummy.num_group = 1;
	driver	  = new e3sm_io_driver_hdf5 (&dummy);

	/* create a new NC file */
	err = driver->create (outfname, MPI_COMM_WORLD, info, &ncid);
	if (err != 0) {
		// printf ("Error at line %d in %s: %s\n", __LINE__, __FILE__, driver->strerrno (err));
		goto fn_exit;
	}
	...
	for (i = 0; i < num_decomp; i++) {
		err = add_decomp (driver, ncid, infname[i], i + 1);
		ERR;
