...
static int write_small_vars_F_case_nc4 (int ncid,
    ...
    if (rec_no == 0) {
        /* lev */
        err = nc_put_var_double (ncid, varids[i++], *dbl_buf);
        NC4ERR
        *dbl_buf += lev + gap;
        /* hyam */
        err = nc_put_var_double (ncid, varids[i++], *dbl_buf);
    ...
    /* time */
    start[0] = rec_no;
    err      = nc_put_var1_double (ncid, varids[i++], start, *dbl_buf);
    NC4ERR
    *dbl_buf += 1 + gap;
    /* date */
    start[0] = rec_no;
    err      = nc_put_var1_int (ncid, varids[i++], start, *int_buf);
    NC4ERR
    *int_buf += 1;
    /* datesec */
    start[0] = rec_no;
    ...
    *int_buf += 1;
    ...
    count[0] = 1;
    count[1] = nbnd;
    err      = nc_put_vara_double (ncid, varids[i++], start, count, *dbl_buf);
    NC4ERR
    *dbl_buf += nbnd + gap;
    /* date_written */
    start[0] = rec_no;
    ...
    count[0] = 1;
    count[1] = nchars;
    err      = nc_put_vara_text (ncid, varids[i++], start, count, *txt_buf);
    NC4ERR
    *txt_buf += nchars;
    /* time_written */
    start[0] = rec_no;
    ...
    *txt_buf += nchars;
    ...
    if (rec_no == 0) {
        /* ndbase */
        err = nc_put_var_int (ncid, varids[i++], *int_buf);
        NC4ERR
        *int_buf += 1;
        /* nsbase */
        err = nc_put_var_int (ncid, varids[i++], *int_buf);
...
static int read_small_vars_F_case_nc4 (int ncid,
                                       int vid, /* starting variable ID */
                                       int *varids,
                                       int rec_no,
                                       int gap,
    ...
    if (rec_no == 0) {
        /* lev */
        err = nc_get_var_double (ncid, varids[i++], *dbl_buf);
        NC4ERR
        *dbl_buf += lev + gap;
        /* hyam */
        err = nc_get_var_double (ncid, varids[i++], *dbl_buf);
    ...
    /* time */
    start[0] = rec_no;
    err      = nc_get_var1_double (ncid, varids[i++], start, *dbl_buf);
    NC4ERR
    *dbl_buf += 1 + gap;
    /* date */
    start[0] = rec_no;
    err      = nc_get_var1_int (ncid, varids[i++], start, *int_buf);
    NC4ERR
    *int_buf += 1;
    /* datesec */
    start[0] = rec_no;
    ...
    *int_buf += 1;
    ...
    count[0] = 1;
    count[1] = nbnd;
    err      = nc_get_vara_double (ncid, varids[i++], start, count, *dbl_buf);
    NC4ERR
    *dbl_buf += nbnd + gap;
    /* date_written */
    start[0] = rec_no;
    ...
    count[0] = 1;
    count[1] = nchars;
    err      = nc_get_vara_text (ncid, varids[i++], start, count, *txt_buf);
    NC4ERR
    *txt_buf += nchars;
    /* time_written */
    start[0] = rec_no;
    ...
    *txt_buf += nchars;
    ...
    if (rec_no == 0) {
        /* ndbase */
        err = nc_get_var_int (ncid, varids[i++], *int_buf);
        NC4ERR
        *int_buf += 1;
        /* nsbase */
        err = nc_get_var_int (ncid, varids[i++], *int_buf);
...
#define POST_VARN(k, num, vid)                                                                  \
    for (j = 0; j < num; j++) {                                                                 \
        err = nc_put_varn (ncid, vid + j, xnreqs[k - 1], starts_D##k, counts_D##k, rec_buf_ptr, \
                           -1, REC_ITYPE);                                                      \
        NC4ERR                                                                                  \
...
int run_varn_F_case_nc4 (
    ...
    MPI_Info info,
    MPI_Offset mpidims[3][2], /* dimension lengths */
    ...
    int *int_buf)             /* buffer for int var */
{
    ...
    size_t **starts_D2 = NULL, **counts_D2 = NULL;
    size_t **starts_D3 = NULL, **counts_D3 = NULL;
    ...
    if (nvars == 408) {
        /* for h0 file */
        err = def_F_case_h0_nc4 (ncid, dims[2], nvars, varids);
        NC4ERR
    } else {
        /* for h1 file */
        err = def_F_case_h1_nc4 (ncid, dims[2], nvars, varids);
        NC4ERR
    }
    ...
    /* I/O amount so far */
    // err = nc_inq_put_size(ncid, &metadata_size); NC4ERR
    // err = nc_inq_file_info(ncid, &info_used); NC4ERR
    ...
    if (xnreqs[1] > 0) {
        ...
        err = nc_put_varn (ncid, varids[i++], xnreqs[1], fix_starts_D2, fix_counts_D2, dbl_buf_ptr,
                           nelems[1], MPI_DOUBLE);
        NC4ERR
    ...
    if (xnreqs[0] > 0) {
        ...
        err = nc_put_varn (ncid, varids[i++], xnreqs[0], fix_starts_D1, fix_counts_D1, dbl_buf_ptr,
                           nelems[0], MPI_DOUBLE);
        NC4ERR
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        // err = nc_wait_all(ncid, NC_REQ_ALL); NC4ERR
    ...
    // err = nc_inq_put_size(ncid, &total_size); NC4ERR
...
#define POST_VARN_RD(k, num, vid)                                                                  \
    for (j = 0; j < num; j++) {                                                                    \
        err = nc_get_varn (ncid, vid + j, nreqs[k - 1], starts_D##k, counts_D##k, rec_buf_ptr, -1, \
                           REC_ITYPE);                                                             \
        NC4ERR                                                                                     \
        rec_buf_ptr += nelems[k - 1] + gap;                                                        \
        my_nreqs += nreqs[k - 1];                                                                  \
    ...
    int *int_buf)             /* buffer for int var */
{
    ...
    NC4ERR NC4ERR
        ...
        if (nvars == 408) {
        /* for h0 file */
        err = inq_F_case_h0_nc4 (ncid, dims[2], nvars, varids);
        NC4ERR
    }
    else {
        /* for h1 file */
        err = inq_F_case_h1_nc4 (ncid, dims[2], nvars, varids);
        NC4ERR
    }
    ...
    /* I/O amount so far */
    // err = nc_inq_get_size(ncid, &metadata_size); NC4ERR
    // err = nc_inq_file_info(ncid, &info_used); NC4ERR
    open_timing += MPI_Wtime () - timing;
    ...
    if (nreqs[1] > 0) {
        ...
        err = nc_get_varn (ncid, varids[i++], nreqs[1], fix_starts_D2, fix_counts_D2, dbl_buf_ptr,
                           nelems[1], MPI_DOUBLE);
        NC4ERR
        dbl_buf_ptr += nelems[1] + gap;
        my_nreqs += nreqs[1];
    ...
    if (nreqs[0] > 0) {
        ...
        err = nc_get_varn (ncid, varids[i++], nreqs[0], fix_starts_D1, fix_counts_D1, dbl_buf_ptr,
                           nelems[0], MPI_DOUBLE);
        NC4ERR
        dbl_buf_ptr += nelems[0] + gap;
        my_nreqs += nreqs[0];
    ...
    // err = nc_inq_get_size(ncid, &total_size); NC4ERR
    put_size = total_size - metadata_size;
    err      = nc_close (ncid);
    NC4ERR
    close_timing += MPI_Wtime () - timing;
