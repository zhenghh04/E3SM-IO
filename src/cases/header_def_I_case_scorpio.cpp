...
#define DEF_VAR(name, type, ndims, dimids) {                                                 \
    err = e3sm_io_scorpio_define_var (                                                       \
        driver, cfg, dnames, decom, v_decomp_ids[(varid - varids)],                              \
        (v_decomp_ids[(varid - varids)] >= 0 ? piodecomid_inv[v_decomp_ids[(varid - varids)]] : -1), \
...
#define PUT_ATTR_INT1(name, buf)                                                           \
    {                                                                                          \
        err = e3sm_io_scorpio_put_att (driver, ncid, *varid, name, NC_INT, 1, (void *)buf); \
        CHECK_VAR_ERR (*varid)                                                                 \
    }
...
#define PUT_ATTR_FLOAT(name, num, buf)                                                           \
    {                                                                                            \
        float flt = buf;   \
        err = e3sm_io_scorpio_put_att (driver, ncid, *varid, name, NC_FLOAT, num, (void *)(&flt)); \
        CHECK_VAR_ERR (*varid)                                                                   \
    }
               ...
               int             ncid)
{
    ...
    // PIO attributes
    PUT_GATTR_INT1 ("/__pio__/fillmode", 256);

    /* 28 global attributes: */
    PUT_GATTR_TXT("pio_global/title", "ELM History file information")
    PUT_GATTR_TXT("pio_global/source", "E3SM Land Model")
    PUT_GATTR_TXT("pio_global/source_id", "6eb829238a")
    PUT_GATTR_TXT("pio_global/product", "model-output")
    PUT_GATTR_TXT("pio_global/realm", "land")
    PUT_GATTR_TXT("pio_global/case", "I1850GSWCNPRDCTCBC_hcru_hcru")
    PUT_GATTR_TXT("pio_global/username", "E3SM")
    PUT_GATTR_TXT("pio_global/hostname", "cori-knl")
    PUT_GATTR_TXT("pio_global/git_version", "6eb829238a")
    PUT_GATTR_TXT("pio_global/history", "created on 07/13/21 20:37:15")
    ...
    PUT_GATTR_TXT("pio_global/comment", "NOTE: None of the variables are weighted by land fraction!")
    PUT_GATTR_TXT("pio_global/Surface_dataset", "surfdata_360x720cru_simyr1850_c180216.nc")
    PUT_GATTR_TXT("pio_global/Initial_conditions_dataset", "arbitrary initialization")
    PUT_GATTR_TXT("pio_global/PFT_physiological_constants_dataset", "clm_params_c180524.nc")

    PUT_GATTR_INT1("pio_global/ltype_vegetated_or_bare_soil", 1)
    PUT_GATTR_INT1("pio_global/ltype_crop", 2)
    PUT_GATTR_INT1("pio_global/ltype_landice", 3)
    PUT_GATTR_INT1("pio_global/ltype_landice_multiple_elevation_classes", 4)
    PUT_GATTR_INT1("pio_global/ltype_deep_lake", 5)
    PUT_GATTR_INT1("pio_global/ltype_wetland", 6)
    PUT_GATTR_INT1("pio_global/ltype_urban_tbd", 7)
    PUT_GATTR_INT1("pio_global/ltype_urban_hd", 8)
    PUT_GATTR_INT1("pio_global/ltype_urban_md", 9)

    if (cfg.nvars == 552) { /* h1 only */
        PUT_GATTR_TXT("pio_global/Time_constant_3Dvars_filenamae", "./I1850GSWCNPRDCTCBC_hcru_hcru.elm.h0.0001-01-01-00000.nc")
        PUT_GATTR_TXT("pio_global/Time_constant_3Dvars", "ZSOI:DZSOI:WATSAT:SUCSAT:BSW:HKSAT:ZLAKE:DZLAKE")
    }
               ...
               int *scorpiovars)
{
    ...
    e3sm_io_scorpio_var *varid = varids;
    ...
    wr_buf->fix_buflen += decom.raw_nreqs[0];
    ...
    PUT_ATTR_DECOMP(one, 2, orig_dimids[0]+1)
    wr_buf->fix_int_buflen += decom.raw_nreqs[0];
    ...
    if (cfg.nvars == 560) {  /* h0 only */
        ...
        ndims = ((cfg.strategy == blob) && false) ? 1 : 3;
        ...
        wr_buf->fix_buflen += decom.raw_nreqs[1];
        ...
        wr_buf->fix_buflen += decom.raw_nreqs[2];
    ...
    wr_buf->rec_buflen += decom.raw_nreqs[0];
    ...
    ndims = ((cfg.strategy == blob) && false) ? 2 : 4;
    ...
    wr_buf->rec_buflen += decom.raw_nreqs[1];
    ...
    wr_buf->rec_buflen += decom.raw_nreqs[2];
    ...
    wr_buf->rec_buflen += decom.raw_nreqs[3];
    ...
    wr_buf->rec_buflen += decom.raw_nreqs[4];
