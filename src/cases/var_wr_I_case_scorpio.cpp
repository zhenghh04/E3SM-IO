...
#define CHECK_VAR_ERR(varid) {                                \
    if (err != 0) {                                           \
        char var_name[64];                                    \
        driver.inq_var_name(ncid, varids[varid].data, var_name);           \
...
#define IPUT_VAR_VTYPE(varid, buf, adv) { \
    err = e3sm_io_scorpio_write_var (driver, rec_no, ncid, varids[varid], REC_ITYPE, buf, nb); \
    CHECK_VAR_ERR(varid) \
    buf += (adv); \
    my_nreqs++; \
}
#define IPUT_VAR_DBL(buf, adv) { \
    err = e3sm_io_scorpio_write_var (driver, rec_no, ncid, varids[varid], MPI_DOUBLE, buf, nb); \
    CHECK_VAR_ERR(i) \
    buf += (adv); \
    my_nreqs++; \
    i++; \
...
#define IPUT_VAR_INT(buf, adv) { \
    err = e3sm_io_scorpio_write_var (driver, rec_no, ncid, varids[varid], MPI_INT, buf, nb); \
...
#define IPUT_VARA_INT64_NOADV(buf) { \
    err = e3sm_io_scorpio_write_var (driver, rec_no, ncid, varids[varid], MPI_LONG_LONG, buf, nb); \
...
#define IPUT_VARA_TXT(varid, buf, adv) { \
    err = e3sm_io_scorpio_write_var (driver, rec_no, ncid, varids[varid], MPI_CHAR, buf, nb); \
    CHECK_VAR_ERR(varid) \
    buf += (adv); \
    my_nreqs++; \
}
...
#define FIX_VAR_IPUT(varid, itype, buf) {                               \
    ...
    err = e3sm_io_scorpio_write_var (driver, rec_no, ncid, varids[vid], itype, buf, nb); \
    CHECK_VAR_ERR(vid)                                                  \
...
static
int write_small_fix_vars_I_case_scorpio(e3sm_io_driver  &driver,
                                int              ncid,
                                int              start_varid,
                                int              gap,
                                vtype         **fix_buf,
                                ...
                                int rec_no)
{
    ...
    int small_fix_varids[5] = {0, 1, 2, 12, 13};

    int i, err, my_nreqs=0;
    MPI_Offset lat, lon, levgrnd, levdcmp, levlak;

    for (i=0; i<18; i++) small_fix_varids[i] += start_varid;
    ...
    /* 5 of type vtype */
    IPUT_VAR_VTYPE(small_fix_varids[0], *fix_buf,  levgrnd + gap) /* levgrnd */
    IPUT_VAR_VTYPE(small_fix_varids[1], *fix_buf,  levlak  + gap) /* levlak */
    IPUT_VAR_VTYPE(small_fix_varids[2], *fix_buf,  levdcmp + gap) /* levdcmp */
    IPUT_VAR_VTYPE(small_fix_varids[3], *fix_buf,  lon     + gap) /* lon */
    IPUT_VAR_VTYPE(small_fix_varids[4], *fix_buf,  lat     + gap) /* lat */
...
static
int write_small_rec_vars_I_case_scorpio(e3sm_io_driver  &driver,
                                int              ncid,
                                int              start_varid,
                                int              rec_no,
                                int              gap,
                                ...
                                e3sm_io_scorpio_var *varids)
{
    ...
    int small_rec_varids[9] = {3, 4, 5, 6, 7, 8, 9, 10, 11};

    int i, err, my_nreqs=0;
    MPI_Offset hist_interval, string_length, start[2], count[2];

    for (i=0; i<9; i++) small_rec_varids[i] += start_varid;
    ...
    /* 1 of type vtype */
    IPUT_VAR1_VTYPE(small_rec_varids[0], *buf,  1 + gap) /* time */

    /* 5 of type int */
    IPUT_VAR1_INT(small_rec_varids[1], *int_buf, 1 + gap) /* mcdate */
    IPUT_VAR1_INT(small_rec_varids[2], *int_buf, 1 + gap) /* mcsec */
    IPUT_VAR1_INT(small_rec_varids[3], *int_buf, 1 + gap) /* mdcur */
    IPUT_VAR1_INT(small_rec_varids[4], *int_buf, 1 + gap) /* mscur */
    IPUT_VAR1_INT(small_rec_varids[5], *int_buf, 1 + gap) /* nstep */

    /* 1 of type double */
    count[1] = hist_interval;
    IPUT_VARA_DBL(small_rec_varids[6], *dbl_buf, hist_interval + gap) /* time_bounds */

    /* 2 of type char */
    count[1] = string_length;
    IPUT_VARA_TXT(small_rec_varids[7], *txt_buf, string_length + gap) /* date_written */
    IPUT_VARA_TXT(small_rec_varids[8], *txt_buf, string_length + gap) /* time_written */
...
/*----< var_wr_I_case() >----------------------------------------------------*/
int var_wr_I_case_scorpio(e3sm_io_config &cfg,
                  e3sm_io_decom  &decom,
                  e3sm_io_driver &driver)
{
    ...
    int rec_no = -1, gap=0, my_nreqs, num_decomp_vars, *v_decomp_ids=NULL;
    ...
    io_buffers wr_buf;
    e3sm_io_scorpio_var *varids = NULL;
    int scorpiovars[6];
...
#define FLUSH_ALL_RECORDS_AT_ONCE
    ...
    err = def_I_case_scorpio(cfg, decom, driver, ncid, v_decomp_ids, &wr_buf, varids, scorpiovars);
    CHECK_ERR
    ...
    // Add 64 for packed start and count
    ...
    if (sub_rank == 0) {
        ...
        err = write_small_fix_vars_I_case_scorpio(driver, ncid, num_decomp_vars, gap,
                                          &fix_buf_ptr, &my_nreqs, varids, rec_no);
        CHECK_ERR
    }
    ...
    for (rec_no=0; rec_no<cfg.nrecs; rec_no++) {
        ...
        if (sub_rank == 0) {
            ...
            err = write_small_rec_vars_I_case_scorpio(driver, ncid, num_decomp_vars,
                                              rec_no, gap,
                                              &rec_txt_buf_ptr,
                                              &rec_int_buf_ptr,
                                              &rec_dbl_buf_ptr,
