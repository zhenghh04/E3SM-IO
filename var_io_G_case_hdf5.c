    ...
    double *D1_fix_dbl_bufp) /* D1 fix double buffer */
{
    ...
    */
    MPI_Offset m_alloc = 0, max_alloc = 0;
    ...
    double **D1_fix_dbl_bufp) /* D1 fix double buffer */
{
    ...
    /* define dimensions, variables, and attributes */
    err = inq_G_case_h0_hdf5 (ncid, dims[0], dims[1], dims[2], dims[3], dims[4], dims[5], nvars,
                              varids);
    ERR
    ...
    /* int maxLevelEdgeTop(nEdges) */
    err = HDF5_IGET_VARN (ncid, 8, nreqs[1], fix_starts_D2, fix_counts_D2, D2_fix_int_buf,
                          nelems[1], MPI_INT, NULL);
    ERR
    my_nreqs += nreqs[1];
    ...
    /* int edgeMask(nEdges, nVertLevels) */
    err = HDF5_IGET_VARN (ncid, 10, nreqs[3], fix_starts_D4, fix_counts_D4, D4_fix_int_buf,
                          nelems[3], MPI_INT, NULL);
    ERR
    my_nreqs += nreqs[3];

    /* int cellMask(nCells, nVertLevels) */
    err = HDF5_IGET_VARN (ncid, 11, nreqs[2], fix_starts_D3, fix_counts_D3, D3_fix_int_buf,
                          nelems[2], MPI_INT, NULL);
    ERR
    my_nreqs += nreqs[2];

    /* int vertexMask(nVertices, nVertLevels) */
    err = HDF5_IGET_VARN (ncid, 12, nreqs[4], fix_starts_D5, fix_counts_D5, D5_fix_int_buf,
                          nelems[4], MPI_INT, NULL);
    ERR
    my_nreqs += nreqs[4];

    /* double bottomDepth(nCells)  */
    err = HDF5_IGET_VARN (ncid, 35, nreqs[0], fix_starts_D1, fix_counts_D1, D1_fix_dbl_buf,
                          nelems[0], MPI_DOUBLE, NULL);
    ERR
    my_nreqs += nreqs[0];

    /* int maxLevelCell(nCells) */
    err = HDF5_IGET_VARN (ncid, 36, nreqs[0], fix_starts_D1, fix_counts_D1, D1_fix_int_buf,
                          nelems[0], MPI_INT, NULL);
    ERR
    my_nreqs += nreqs[0];
    ...
    if (rank == 0) {
        ...
        /* double vertCoordMovementWeights(nVertLevels) */
        err = HDF5_IGET_VARS_DOUBLE (ncid, 9, start, count, stride, dummy_double_buf, NULL);
        ERR

        /* double refZMid(nVertLevels) */
        err = HDF5_IGET_VARS_DOUBLE (ncid, 13, start, count, stride, dummy_double_buf, NULL);
        ERR

        /* double refLayerThickness(nVertLevels) */
        err = HDF5_IGET_VARS_DOUBLE (ncid, 14, start, count, stride, dummy_double_buf, NULL);
        ERR

        /* double refBottomDepth(nVertLevels) */
        err = HDF5_IGET_VARS_DOUBLE (ncid, 33, start, count, stride, dummy_double_buf, NULL);
        ERR
        ...
        for (rec_no = 0; rec_no < num_recs; rec_no++) {
            ...
            /* char xtime(Time, StrLen) */
            err = HDF5_IGET_VARS_TEXT (ncid, 15, start, count, stride, dummy_char_buf, NULL);
            ERR

            /* double areaCellGlobal(Time) */
            err = HDF5_IGET_VARS_DOUBLE (ncid, 20, start, count, stride, dummy_double_buf, NULL);
            ERR

            /* double areaEdgeGlobal(Time) */
            err = HDF5_IGET_VARS_DOUBLE (ncid, 21, start, count, stride, dummy_double_buf, NULL);
            ERR

            /* double areaTriangleGlobal(Time) */
            err = HDF5_IGET_VARS_DOUBLE (ncid, 22, start, count, stride, dummy_double_buf, NULL);
            ERR

            /* double volumeCellGlobal(Time) */
            err = HDF5_IGET_VARS_DOUBLE (ncid, 23, start, count, stride, dummy_double_buf, NULL);
            ERR

            /* double volumeEdgeGlobal(Time) */
            err = HDF5_IGET_VARS_DOUBLE (ncid, 24, start, count, stride, dummy_double_buf, NULL);
            ERR

            /* double CFLNumberGlobal(Time) */
            err = HDF5_IGET_VARS_DOUBLE (ncid, 25, start, count, stride, dummy_double_buf, NULL);
            ERR
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        rec_buf_ptr = D1_rec_dbl_buf;
        for (j = 0; j < nD1_rec_2d_vars; j++) {
            err = HDF5_IGET_VARN (ncid, D1_rec_2d_varids[j], nreqs[0], starts_D1, counts_D1,
                                  rec_buf_ptr, nelems[0], MPI_DOUBLE, NULL);
            ERR
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        rec_buf_ptr = D6_rec_dbl_buf;
        for (j = 0; j < nD6_rec_3d_vars; j++) {
            err = HDF5_IGET_VARN (ncid, D6_rec_3d_varids[j], nreqs[5], starts_D6, counts_D6,
                                  rec_buf_ptr, nelems[5], MPI_DOUBLE, NULL);
            ERR
            rec_buf_ptr += nelems[5];
            my_nreqs += nreqs[5];
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        rec_buf_ptr = D3_rec_dbl_buf;
        for (j = 0; j < nD3_rec_3d_vars; j++) {
            err = HDF5_IGET_VARN (ncid, D3_rec_3d_varids[j], nreqs[2], starts_D3, counts_D3,
                                  rec_buf_ptr, nelems[2], MPI_DOUBLE, NULL);
            ERR
            rec_buf_ptr += nelems[2];
            my_nreqs += nreqs[2];
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        rec_buf_ptr = D4_rec_dbl_buf;
        for (j = 0; j < nD4_rec_3d_vars; j++) {
            err = HDF5_IGET_VARN (ncid, D4_rec_3d_varids[j], nreqs[3], starts_D4, counts_D4,
                                  rec_buf_ptr, nelems[3], MPI_DOUBLE, NULL);
            ERR
            rec_buf_ptr += nelems[3];
            my_nreqs += nreqs[3];
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        rec_buf_ptr = D5_rec_dbl_buf;
        for (j = 0; j < nD5_rec_3d_vars; j++) {
            err = HDF5_IGET_VARN (ncid, D5_rec_3d_varids[j], nreqs[4], starts_D5, counts_D5,
                                  rec_buf_ptr, nelems[4], MPI_DOUBLE, NULL);
            ERR
            rec_buf_ptr += nelems[4];
            my_nreqs += nreqs[4];
