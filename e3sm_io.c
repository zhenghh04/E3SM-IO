...
int main (int argc, char **argv) {
    ...
    int i, rank, nprocs, err, nerrs = 0, tst_vard = 0, tst_varn = 0, tst_wr = 0, tst_rd = 0,
                              noncontig_buf = 0, api = E3SM_API_PNC;
...
#else
            if (tst_rd) {
                if (!rank) printf ("Reading isnot supported for vard\n");
            }
            ...
            if (tst_wr) {
                if (api == E3SM_API_PNC) {
                    ...
                    nerrs +=
                        run_vard_F_case (io_comm, out_dir, outfname, nvars, num_recs, noncontig_buf,
                                         info, dims, contig_nreqs, disps, blocklens);
                } else if (api == E3SM_API_NC4) {
                    printf ("NetCDF4 does not support vard, skip\n");
                }
            }
#endif
        }
        if (tst_varn) {
            if (tst_rd) {
                ...
                if (api == E3SM_API_PNC) {
                    ...
                    nvars    = 408;
                    outfname = "f_case_h0_varn.nc";
                    nerrs += run_varn_F_case_rd_nc4 (io_comm, in_dir, outfname, nvars, num_recs,
                                                     noncontig_buf, info, dims, contig_nreqs, disps,
            ...
            if (tst_wr) {
                ...
                if (api == E3SM_API_PNC) {
                    ...
                    nvars    = 408;
                    outfname = "f_case_h0_varn.nc";
                    nerrs += run_varn_F_case_nc4 (io_comm, out_dir, outfname, nvars, num_recs,
                                                  noncontig_buf, info, dims, contig_nreqs, disps,
                                                  blocklens, dbl_buf_h0, rec_buf_h0, txt_buf[0],
                                                  int_buf[0]);
