...
#define FIX_1D_VAR_STARTS_COUNTS(starts, counts, nreqs, disps, blocklens) \
    {                                                                     \
        starts    = (size_t **)malloc (2 * nreqs * sizeof (size_t *));    \
        counts    = starts + nreqs;                                       \
        starts[0] = (size_t *)malloc (2 * nreqs * sizeof (size_t));       \
...
#define FIX_2D_VAR_STARTS_COUNTS(starts, counts, nreqs, disps, blocklens, last_dimlen)             \
    {                                                                                              \
        starts    = (size_t **)malloc (2 * nreqs * sizeof (size_t *));                             \
        counts    = starts + nreqs;                                                                \
        starts[0] = (size_t *)malloc (2 * nreqs * 2 * sizeof (size_t));                            \
...
#define REC_3D_VAR_STARTS_COUNTS(rec, starts, counts, nreqs, disps, blocklens, last_dimlen)        \
    {                                                                                              \
        starts    = (size_t **)malloc (2 * nreqs * sizeof (size_t *));                             \
        counts    = starts + nreqs;                                                                \
        starts[0] = (size_t *)malloc (2 * nreqs * 3 * sizeof (size_t));                            \
...
int run_varn_G_case_nc4 (
    ...
    MPI_Info info,
    MPI_Offset mpidims[6][2], /* dimension lengths decomposition 1-6 */
    ...
    double *D1_fix_dbl_bufp)  /* D1 fix double buffer */
{
    ...
    size_t **fix_starts_D1, **fix_counts_D1;
    size_t **fix_starts_D2, **fix_counts_D2;
    size_t **fix_starts_D3, **fix_counts_D3;
    size_t **fix_starts_D4, **fix_counts_D4;
    size_t **fix_starts_D5, **fix_counts_D5;
    size_t **starts_D1, **counts_D1;
    size_t **starts_D3, **counts_D3;
    size_t **starts_D4, **counts_D4;
    size_t **starts_D5, **counts_D5;
    size_t **starts_D6, **counts_D6;
    ...
    for (i = 0; i < 6; i++) {
        for (j = 0; j < 2; j++) { dims[i][j] = (size_t)mpidims[i][j]; }
    }
    ...
    err   = nc_create_par (outfname, cmode, io_comm, info, &ncid);
    NC4ERR

    MPI_Offset put_buffer_size_limit = 10485760;
    // err                              = nc_buffer_attach (ncid, put_buffer_size_limit);
    // NC4ERR

    /* define dimensions, variables, and attributes */
    err = def_G_case_h0_nc4 (ncid, dims[0], dims[1], dims[2], dims[3], dims[4], dims[5], nvars,
                             varids);
    NC4ERR
    ...
    /* I/O amount so far */
    // err = nc_inq_put_size (ncid, &metadata_size);
    // NC4ERR
    // err = nc_inq_file_info (ncid, &info_used);
    // NC4ERR
    ...
    err = nc_put_varn (ncid, 8, xnreqs[1], fix_starts_D2, fix_counts_D2, D2_fix_int_buf, nelems[1],
                       MPI_INT);
    NC4ERR
    my_nreqs += xnreqs[1];
    nvars_D[1]++;
    ...
    err = nc_put_varn (ncid, 37, xnreqs[1], fix_starts_D2, fix_counts_D2,
    ...
    err = nc_put_varn (ncid, 10, xnreqs[3], fix_starts_D4, fix_counts_D4, D4_fix_int_buf, nelems[3],
                       MPI_INT);
    NC4ERR
    my_nreqs += xnreqs[3];
    nvars_D[3]++;
    ...
    err = nc_put_varn (ncid, 11, xnreqs[2], fix_starts_D3, fix_counts_D3, D3_fix_int_buf, nelems[2],
                       MPI_INT);
    NC4ERR
    my_nreqs += xnreqs[2];
    nvars_D[2]++;
    ...
    err = nc_put_varn (ncid, 12, xnreqs[4], fix_starts_D5, fix_counts_D5, D5_fix_int_buf, nelems[4],
                       MPI_INT);
    NC4ERR
    my_nreqs += xnreqs[4];
    nvars_D[4]++;
    ...
    err = nc_put_varn (ncid, 35, xnreqs[0], fix_starts_D1, fix_counts_D1, D1_fix_dbl_buf, nelems[0],
                       MPI_DOUBLE);
    NC4ERR
    my_nreqs += xnreqs[0];
    nvars_D[0]++;
    ...
    err = nc_put_varn (ncid, 36, xnreqs[0], fix_starts_D1, fix_counts_D1, D1_fix_int_buf, nelems[0],
                       MPI_INT);
    NC4ERR
    my_nreqs += xnreqs[0];
    nvars_D[0]++;
    ...
    if (rank == 0) {
        ...
        err = nc_put_vars_double (ncid, 9, start, count, stride, dummy_double_buf);
        NC4ERR

        /* double refZMid(nVertLevels) */
        err = nc_put_vars_double (ncid, 13, start, count, stride, dummy_double_buf);
        NC4ERR

        /* double refLayerThickness(nVertLevels) */
        err = nc_put_vars_double (ncid, 14, start, count, stride, dummy_double_buf);
        NC4ERR

        /* double refBottomDepth(nVertLevels) */
        err = nc_put_vars_double (ncid, 33, start, count, stride, dummy_double_buf);
        NC4ERR
        ...
        for (rec_no = 0; rec_no < num_recs; rec_no++) {
            ...
            err = nc_put_vars_text (ncid, 15, start, count, stride, dummy_char_buf);
            NC4ERR

            /* double areaCellGlobal(Time) */
            err = nc_put_vars_double (ncid, 20, start, count, stride, dummy_double_buf);
            NC4ERR

            /* double areaEdgeGlobal(Time) */
            err = nc_put_vars_double (ncid, 21, start, count, stride, dummy_double_buf);
            NC4ERR

            /* double areaTriangleGlobal(Time) */
            err = nc_put_vars_double (ncid, 22, start, count, stride, dummy_double_buf);
            NC4ERR

            /* double volumeCellGlobal(Time) */
            err = nc_put_vars_double (ncid, 23, start, count, stride, dummy_double_buf);
            NC4ERR

            /* double volumeEdgeGlobal(Time) */
            err = nc_put_vars_double (ncid, 24, start, count, stride, dummy_double_buf);
            NC4ERR

            /* double CFLNumberGlobal(Time) */
            err = nc_put_vars_double (ncid, 25, start, count, stride, dummy_double_buf);
            NC4ERR
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        for (j = 0; j < nD1_rec_2d_vars; j++) {
            err = nc_put_varn (ncid, D1_rec_2d_varids[j], xnreqs[0], starts_D1, counts_D1,
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        for (j = 0; j < nD6_rec_3d_vars; j++) {
            err = nc_put_varn (ncid, D6_rec_3d_varids[j], xnreqs[5], starts_D6, counts_D6,
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        for (j = 0; j < nD3_rec_3d_vars; j++) {
            err = nc_put_varn (ncid, D3_rec_3d_varids[j], xnreqs[2], starts_D3, counts_D3,
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        for (j = 0; j < nD4_rec_3d_vars; j++) {
            err = nc_put_varn (ncid, D4_rec_3d_varids[j], xnreqs[3], starts_D4, counts_D4,
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        for (j = 0; j < nD5_rec_3d_vars; j++) {
            err = nc_put_varn (ncid, D5_rec_3d_varids[j], xnreqs[4], starts_D5, counts_D5,
    ...
    // err = nc_wait_all (ncid, NC_REQ_ALL);
    NC4ERR
    ...
    // err = nc_inq_put_size (ncid, &total_size);
    // NC4ERR
    put_size = total_size - metadata_size;
    // err      = nc_buffer_detach (ncid);
    // NC4ERR
    err = nc_close (ncid);
    NC4ERR
    ...
    if (rank == 0) {
        ...
        printf ("Max Time of nc_put_varn        = %.4f sec\n", post_timing);
        // printf ("Max Time of nc_wait_all         = %.4f sec\n", wait_timing);
    ...
    double **D1_fix_dbl_bufp) /* D1 fix double buffer */
{
    ...
    /* create a new CDF-5 file for writing */
    cmode = NC_NOWRITE;
    err   = nc_open_par (outfname, cmode, io_comm, info, &ncid);
    NC4ERR
    ...
    /* define dimensions, variables, and attributes */
    err = inq_G_case_h0_nc4 (ncid, dims[0], dims[1], dims[2], dims[3], dims[4], dims[5], nvars,
                             varids);
    NC4ERR

    /* exit define mode and enter data mode */
    // err = nc_enddef(ncid); NC4ERR

    /* I/O amount so far */
    // err = nc_inq_get_size (ncid, &metadata_size);
    // NC4ERR
    // err = nc_inq_file_info (ncid, &info_used);
    // NC4ERR
    open_timing += MPI_Wtime () - timing;
    ...
    /* int maxLevelEdgeTop(nEdges) */
    err = nc_get_varn (ncid, 8, nreqs[1], fix_starts_D2, fix_counts_D2, D2_fix_int_buf, nelems[1],
                       MPI_INT);
    NC4ERR
    my_nreqs += nreqs[1];

    /* int maxLevelEdgeBot(nEdges) */
    err = nc_get_varn (ncid, 37, nreqs[1], fix_starts_D2, fix_counts_D2, D2_fix_int_buf + nelems[1],
                       nelems[1], MPI_INT);
    NC4ERR
    my_nreqs += nreqs[1];

    /* int edgeMask(nEdges, nVertLevels) */
    err = nc_get_varn (ncid, 10, nreqs[3], fix_starts_D4, fix_counts_D4, D4_fix_int_buf, nelems[3],
                       MPI_INT);
    NC4ERR
    my_nreqs += nreqs[3];

    /* int cellMask(nCells, nVertLevels) */
    err = nc_get_varn (ncid, 11, nreqs[2], fix_starts_D3, fix_counts_D3, D3_fix_int_buf, nelems[2],
                       MPI_INT);
    NC4ERR
    my_nreqs += nreqs[2];

    /* int vertexMask(nVertices, nVertLevels) */
    err = nc_get_varn (ncid, 12, nreqs[4], fix_starts_D5, fix_counts_D5, D5_fix_int_buf, nelems[4],
                       MPI_INT);
    NC4ERR
    my_nreqs += nreqs[4];

    /* double bottomDepth(nCells)  */
    err = nc_get_varn (ncid, 35, nreqs[0], fix_starts_D1, fix_counts_D1, D1_fix_dbl_buf, nelems[0],
                       MPI_DOUBLE);
    NC4ERR
    my_nreqs += nreqs[0];

    /* int maxLevelCell(nCells) */
    err = nc_get_varn (ncid, 36, nreqs[0], fix_starts_D1, fix_counts_D1, D1_fix_int_buf, nelems[0],
                       MPI_INT);
    NC4ERR
    my_nreqs += nreqs[0];
    ...
    if (rank == 0) {
        ...
        /* double vertCoordMovementWeights(nVertLevels) */
        err = nc_get_vars_double (ncid, 9, start, count, stride, dummy_double_buf);
        NC4ERR

        /* double refZMid(nVertLevels) */
        err = nc_get_vars_double (ncid, 13, start, count, stride, dummy_double_buf);
        NC4ERR

        /* double refLayerThickness(nVertLevels) */
        err = nc_get_vars_double (ncid, 14, start, count, stride, dummy_double_buf);
        NC4ERR

        /* double refBottomDepth(nVertLevels) */
        err = nc_get_vars_double (ncid, 33, start, count, stride, dummy_double_buf);
        NC4ERR
        ...
        for (rec_no = 0; rec_no < num_recs; rec_no++) {
            ...
            /* char xtime(Time, StrLen) */
            err = nc_get_vars_text (ncid, 15, start, count, stride, dummy_char_buf);
            NC4ERR

            /* double areaCellGlobal(Time) */
            err = nc_get_vars_double (ncid, 20, start, count, stride, dummy_double_buf);
            NC4ERR

            /* double areaEdgeGlobal(Time) */
            err = nc_get_vars_double (ncid, 21, start, count, stride, dummy_double_buf);
            NC4ERR

            /* double areaTriangleGlobal(Time) */
            err = nc_get_vars_double (ncid, 22, start, count, stride, dummy_double_buf);
            NC4ERR

            /* double volumeCellGlobal(Time) */
            err = nc_get_vars_double (ncid, 23, start, count, stride, dummy_double_buf);
            NC4ERR

            /* double volumeEdgeGlobal(Time) */
            err = nc_get_vars_double (ncid, 24, start, count, stride, dummy_double_buf);
            NC4ERR

            /* double CFLNumberGlobal(Time) */
            err = nc_get_vars_double (ncid, 25, start, count, stride, dummy_double_buf);
            NC4ERR
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        rec_buf_ptr = D1_rec_dbl_buf;
        for (j = 0; j < nD1_rec_2d_vars; j++) {
            err = nc_get_varn (ncid, D1_rec_2d_varids[j], nreqs[0], starts_D1, counts_D1,
                               rec_buf_ptr, nelems[0], MPI_DOUBLE);
            NC4ERR
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        rec_buf_ptr = D6_rec_dbl_buf;
        for (j = 0; j < nD6_rec_3d_vars; j++) {
            err = nc_get_varn (ncid, D6_rec_3d_varids[j], nreqs[5], starts_D6, counts_D6,
                               rec_buf_ptr, nelems[5], MPI_DOUBLE);
            NC4ERR
            rec_buf_ptr += nelems[5];
            my_nreqs += nreqs[5];
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        rec_buf_ptr = D3_rec_dbl_buf;
        for (j = 0; j < nD3_rec_3d_vars; j++) {
            err = nc_get_varn (ncid, D3_rec_3d_varids[j], nreqs[2], starts_D3, counts_D3,
                               rec_buf_ptr, nelems[2], MPI_DOUBLE);
            NC4ERR
            rec_buf_ptr += nelems[2];
            my_nreqs += nreqs[2];
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        rec_buf_ptr = D4_rec_dbl_buf;
        for (j = 0; j < nD4_rec_3d_vars; j++) {
            err = nc_get_varn (ncid, D4_rec_3d_varids[j], nreqs[3], starts_D4, counts_D4,
                               rec_buf_ptr, nelems[3], MPI_DOUBLE);
            NC4ERR
            rec_buf_ptr += nelems[3];
            my_nreqs += nreqs[3];
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        rec_buf_ptr = D5_rec_dbl_buf;
        for (j = 0; j < nD5_rec_3d_vars; j++) {
            err = nc_get_varn (ncid, D5_rec_3d_varids[j], nreqs[4], starts_D5, counts_D5,
                               rec_buf_ptr, nelems[4], MPI_DOUBLE);
            NC4ERR
            rec_buf_ptr += nelems[4];
            my_nreqs += nreqs[4];
    ...
    // err = nc_inq_get_size (ncid, &total_size);
    // NC4ERR
    ...
    if (rank == 0) {
        ...
        printf ("Max Time of open + metadata define = %.4f sec\n", open_timing);
        printf ("Max Time of I/O preparing          = %.4f sec\n", pre_timing);
        printf ("Max Time of nc_get_varn        = %.4f sec\n", post_timing);
        // printf ("Max Time of nc_wait_all         = %.4f sec\n", wait_timing);
        printf ("Max Time of close                  = %.4f sec\n", close_timing);
        printf ("Max Time of TOTAL                  = %.4f sec\n", total_timing);
        printf ("I/O bandwidth (open-to-close)      = %.4f MiB/sec\n",
